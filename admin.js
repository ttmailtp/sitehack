...
jQuery( document ).on('click', '.tcheck', function() {
...
jQuery( document ).on('click', '.reset_data_btn', function() {
...
jQuery( document ).on( 'click', '.save_data_btn', function() {
...
jQuery( document ).on( 'click', '.add_slide_btn', function () {
	var slidenum = 0;
	...
	var ddhtml=`<div class='transparent'></div><div class='inner'><table>
		<tr><td>Image URL:</td><td><input class='tinput finput' type='text' id='up_new_slide_img_${slidenum}' name='custom_slides[${slidenum}][img]' value='${img}' /><span class='gc_imageupload button' id='upb_new_slide_img_${slidenum}'>Upload</span></td></tr>
		<tr><td>Link URL:</td><td><input class='tinput' type='text' name='custom_slides[${slidenum}][link]' id='new_slide_link_${slidenum}' value='${link}' /></td></tr>
		<tr><td>Title:</td><td><input class='tinput' type='text' name='custom_slides[${slidenum}][ttl]' id='new_slide_ttl_${slidenum}' value='${ttl}' /></td></tr>
		<tr><td>Content:</td><td><textarea class='tinput' id='new_slide_content_${slidenum}' name='custom_slides[${slidenum}][content]'>${content}</textarea></td></tr>
		<tr><td></td><td><span class='button delete_slide_btn' style='float:right' alt='${slidenum}'>Delete</span><span class='button save_slide_btn' style='float:right'>Save</span></td></tr>
	</table></div>`;
...
jQuery( document ).on( 'click', '.save_slide_btn', function() {
...
jQuery( document ).on('click', '.delete_slide_btn', function() {
	var id=jQuery(this).attr('alt');
	jQuery( this ).closest( '.slide_item' ).remove();
	jQuery( '#fade' ).click();
});
jQuery( document ).on( "click", '.custom_slides .slide_item > img', function() {
...
jQuery( document ).on( 'change', '.addselect', function() {
...
jQuery( document ).on('change', '.changeselect', function(e) {
...
jQuery( document ).on('click', '.add_detail_btn', function() {
...
jQuery( document ).on('click', '.save_detail_btn', function() {
	var img=jQuery('#up_new_detail_img').val();
	var txt=jQuery('#new_detail_value').val();
...
jQuery( document ).on('click', '.itemedit', function() {
...
jQuery( document ).on('click', '.add_form_btn', function() {
	
	var alt=jQuery(this).attr('alt')-0+1;
	var adv=jQuery('td.advanced:first').css('display')!='none';
	var tr = `<tr alt='${alt}'><td class='trdrag'></td>
	<td style='width:50%'>
	<input type='text' class='tinput' name='contactform[${alt}][ttl]' value='' />
	</td>
	<td style='width:30%' class='advanced'>
	<input type='text' class='tinput' name='contactform[${alt}][regex]' value='' />
	</td>
	<td>
	<select name='contactform[${alt}][type]' class='tselect'>
	<option value='text'>Text field</option>
	<option value='textarea'>Text area</option>
	</select>
	</td>
	<td>
	<input type='checkbox' name='contactform[${alt}][req]' value='required' />
	</td>
	<td>
...
jQuery( document ).on('click', '.itemdelete', function() {
	jQuery(this).parent().remove();
});
jQuery( document ).on('click', '.advanced_settings', function(){
	jQuery('.advanced').slideToggle();
});
jQuery( document ).on('click', '.tableitemdelete', function() {
	jQuery(this).parents('tr').remove();
});

jQuery( document ).on('click', '.selectimg', function(){
	if (jQuery('#detailspreset').css('display')=='none')
		jQuery('#detailspreset').show();
	else jQuery('#detailspreset').hide();
});
jQuery( document ).on('click', '#detailspreset img', function() {
...
jQuery( document ).on( 'click', '.add_social_btn', function() {
	jQuery('#new_social').fadeIn();
});
jQuery( document ).on('click', '.edit_social_btn', function() {
...
jQuery( document ).on('click', '.save_social_btn', function() {
...
jQuery( document ).on( 'change', '.adm-form input', function() {
	jQuery('.save_data_btn').css('background-position', 'left top');
});
jQuery( document ).on( 'change', '.adm-form select', function() {
	jQuery('.save_data_btn').css('background-position', 'left top');
});
jQuery( document ).on( 'click', '.sidebarselector img', function() {
...
jQuery( document ).on( 'click', '.gc_imageupload', function() {
	var fname=Math.floor(Math.random()*10000000),
		input = jQuery( '#' + jQuery( this ).attr('id').replace( /^upb/, 'up' ) )
		image = jQuery( '#' + jQuery( this ).attr('id').replace( /^upb/, 'img' ) )
	
	if( wp.media.frames[ input.attr( 'name' ) ] ) {
		wp.media.frames[ input.attr( 'name' ) ].open();
		return;
	}
	wp.media.frames[ input.attr( 'name' ) ] = wp.media({
		title: 'Select image',
		multiple: false,
		library: {
			type: 'image'
	...
	wp.media.frames[ input.attr( 'name' ) ].on( 'select', function() {
		var selection = wp.media.frames[ input.attr( 'name' ) ].state().get('selection');
...
jQuery(document).ready(function(){
	jQuery( document ).on( 'click', '.new_video', function(){
	...
	jQuery( document ).on( 'click', '.preview_banner', function() {
...
jQuery(document).ready(function(){
	jQuery( document ).on( 'click', '.new_social', function(){
		var get_id=jQuery(this).attr('alt');
	...
	jQuery( document ).on( 'click', '.new_profile .title', function() {
		jQuery(this).next('div').slideToggle();
	});
	jQuery( document ).on( 'click', '.social_presets img', function() {
	...
	jQuery( document ).on( 'click', '.profile_delete', function() {
		jQuery(this).parent().parent().remove();
	});
	jQuery( document ).on( 'click', '.social_presets_btn', function() {
...
jQuery(document).ready(function(){
	jQuery( document ).on( 'click', '.new_banner', function(){
		var get_id=jQuery( this ).attr( 'alt' )
	...
	jQuery( document ).on( 'click', '.delete_banner', function(){
